...
test.describe('Oz.by e2e log in tests', async function () {
  ...
  test('log in with not existing Email with password should fail', async ({ page }) => {
    ...
    await homePage.customClick(await page.locator(homePage.loginButton));
  ...
  test('log in by Invalid Email with optional password should fail', async ({ page }) => {
    ...
    await homePage.customClick(inactiveButton);
  ...
  test('sign up with no phone number should fail', async ({ page }) => {
    ...
    await expect(await page.locator(loginPage.errorLogInMessage)).toContainText( MESSAGE.MUST_CONTAIN_SEVEN_DIGITS);
  });
  ...
  test('sign up with Invalid phone number should fail', async ({ page }) => {
    ...
    await expect(await page.locator(loginPage.errorLogInMessage)).toContainText(MESSAGE.PROVIDE_CORRECT_PHONE_NUMBER);
  });
});
